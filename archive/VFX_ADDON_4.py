...
def _cleanup_fog_nodes(nt):
    """Сносит старый разбросанный туман; группу и ее ноду не трогает."""
    for node in list(nt.nodes):
        n = node.name
        if n == "VFX_FOG_GROUP":
            continue
...
def _apply_global_fog(nt, vfx, current):
    """Туман в САМОМ конце: mist -> ramp -> strength -> поверх всей сцены.
    Белый = максимум плотности, черный = прозрачно."""
    strength = getattr(vfx, "fog_strength", 0.0)
    if strength <= 0.001 or current is None:
        return current
    ...
    ramp = nt.nodes.get("VFX_FOG_RAMP")
    if ramp is None:
        ramp = _new_node(nt, "CompositorNodeValToRGB", "ShaderNodeValToRGB")
        if ramp is not None:
            ramp.name = "VFX_FOG_RAMP"
            ramp.label = "FOG RAMP"
            ramp.location = (-100, 600)
    if ramp is not None:
        fin = ramp.inputs[0]
        for l in list(fin.links):
            nt.links.remove(l)
        nt.links.new(mist, fin)
        ramp_out = ramp.outputs[0]
    else:
        ramp_out = mist

    math_n = nt.nodes.get("VFX_FOGMATH_G")
    if math_n is None:
        math_n = _new_node(nt, "CompositorNodeMath", "ShaderNodeMath")
        if math_n is None:
            return current
        math_n.name = "VFX_FOGMATH_G"
    ...
    for l in list(math_n.inputs[0].links):
        nt.links.remove(l)
    nt.links.new(ramp_out, math_n.inputs[0])

    # весовая карта: факторы слоев композитятся по тем же альфа-краям,
    # что и сама сцена - граница тумана совпадает с границей объектов,
    # без ореолов и без зубцов
    bgf = getattr(vfx, "bg_fog_factor", 1.0)
    w_prev = None
    ...
    def _mix_sockets(nd):
        if nd.bl_idname == 'ShaderNodeMix':
            try:
                ...
                pass
            fac_s = a_s = b_s = out_s = None
            for s in nd.inputs:
                if fac_s is None and s.type == 'VALUE' and s.name == 'Factor':
                    fac_s = s
            ...
            for s in nd.outputs:
                if s.type == 'RGBA':
                    out_s = s
                    break
            return fac_s, a_s, b_s, out_s
        fac_s = nd.inputs.get("Fac")
        a_s = nd.inputs.get("Color1")
        b_s = nd.inputs.get("Color2")
        out_s = nd.outputs[0] if len(nd.outputs) else None
        return fac_s, a_s, b_s, out_s
    ...
    for layer in reversed(vfx.layers):
        ...
        if ln is None or ln.outputs.get("Alpha") is None:
            continue
        f = getattr(layer, "fog_factor", 1.0)
        mw = nt.nodes.get(f"VFX_FOGWM_{layer.id}")
        if mw is None:
            mw = _new_node(nt, "CompositorNodeMixRGB", "ShaderNodeMix")
            if mw is None:
                continue
            mw.name = f"VFX_FOGWM_{layer.id}"
            mw["vfx_fog"] = 1
            mw.location = (200, 1000)
        fac_s, a_s, b_s, out_s = _mix_sockets(mw)
        if fac_s is None or a_s is None or b_s is None or out_s is None:
            continue
        try:
            b_s.default_value = (f, f, f, 1.0)
        except Exception:
            pass
        for l in list(fac_s.links):
            nt.links.remove(l)
        nt.links.new(ln.outputs["Alpha"], fac_s)
        if w_prev is None:
            try:
                a_s.default_value = (bgf, bgf, bgf, 1.0)
            except Exception:
                pass
        else:
            for l in list(a_s.links):
                nt.links.remove(l)
            nt.links.new(w_prev, a_s)
        w_prev = out_s

    if w_prev is None:
        return current

    math2 = nt.nodes.get("VFX_FOGMATH2_G")
    if math2 is None:
        math2 = _new_node(nt, "CompositorNodeMath", "ShaderNodeMath")
        if math2 is None:
            return current
        math2.name = "VFX_FOGMATH2_G"
        math2["vfx_fog"] = 1
        math2.location = (300, 400)
    try:
        math2.operation = 'MULTIPLY'
    except Exception:
        pass
    for l in list(math2.inputs[0].links):
        nt.links.remove(l)
    for l in list(math2.inputs[1].links):
        nt.links.remove(l)
    nt.links.new(math_n.outputs[0], math2.inputs[0])
    nt.links.new(w_prev, math2.inputs[1])
    fac_out = math2.outputs[0]

    # один Mix: factor = маска*strength*вес, A = сцена, B = цвет тумана.
    mix = nt.nodes.get("VFX_FOGMIX_G")
    if mix is None:
        mix = _new_node(nt, "CompositorNodeMixRGB", "ShaderNodeMix")
        if mix is None:
            return current
        mix.name = "VFX_FOGMIX_G"
        mix.label = "FOG MIX"
        mix.location = (420, 400)
        mix["vfx_fog"] = 1
    ...
    if mix.bl_idname == 'ShaderNodeMix':
        ...
        fac_in = mix.inputs.get("Fac")
        a_in = mix.inputs.get("Color1")
        b_in = mix.inputs.get("Color2")
        outs = [s for s in mix.outputs if s.type in ('RGBA', 'VALUE')]
        out_s = outs[0] if outs else None

    if fac_in is None or a_in is None or b_in is None or out_s is None:
        print("VFX fog: mix sockets not found")
        return current

    try:
        r, g, b, a = vfx.fog_color
        b_in.default_value = (r, g, b, 1.0)
    except Exception:
        pass

    for l in list(fac_in.links):
        nt.links.remove(l)
    for l in list(a_in.links):
        nt.links.remove(l)
    nt.links.new(fac_out, fac_in)
    nt.links.new(current, a_in)

    return out_s


def _build_fog_group(vfx):
    """Вся математика тумана внутри отдельной нод-группы."""
    ...
    for layer in reversed(vfx.layers):
        if layer.enabled and layer.scene:
            ...
            ng.interface.new_socket(f"A_{layer.id}", in_out='INPUT',
                                    socket_type='NodeSocketFloat')
            ng.interface.new_socket(f"F_{layer.id}", in_out='INPUT',
                                    socket_type='NodeSocketFloat')
    ng.interface.new_socket("A_BG", in_out='INPUT', socket_type='NodeSocketFloat')
    ...
    ms = math_node('MULTIPLY', (-350, 250))
    if ms is None:
        return ng, layer_ids
    ng.links.new(mr.outputs.get("Result"), ms.inputs[0])
    ng.links.new(g_in("Strength"), ms.inputs[1])

    # весовая карта: w = lerp(w, f_i, a_i), слои снизу вверх
    w_out = g_in("F_BG")
    yy = 0
    for lid in layer_ids:
        a_s = g_in(f"A_{lid}")
        f_s = g_in(f"F_{lid}")
        sub = math_node('SUBTRACT', (-350, yy))
        mul = math_node('MULTIPLY', (-200, yy))
        add = math_node('ADD', (-50, yy))
        if sub is None or mul is None or add is None:
            break
        ng.links.new(f_s, sub.inputs[0])
        ng.links.new(w_out, sub.inputs[1])
        ng.links.new(sub.outputs[0], mul.inputs[0])
        ng.links.new(a_s, mul.inputs[1])
        ng.links.new(w_out, add.inputs[0])
        ng.links.new(mul.outputs[0], add.inputs[1])
        w_out = add.outputs[0]
        yy -= 180

    m2 = math_node('MULTIPLY', (100, 250))
    if m2 is None:
        return ng, layer_ids
    ng.links.new(ms.outputs[0], m2.inputs[0])
    ng.links.new(w_out, m2.inputs[1])
    ...
    if mix.bl_idname == 'ShaderNodeMix':
        ...
        a_in = mix.inputs.get("Color1")
        b_in = mix.inputs.get("Color2")
        out_s = mix.outputs[0] if len(mix.outputs) else None

    if fac_in is not None and a_in is not None and b_in is not None and out_s is not None:
        ng.links.new(m2.outputs[0], fac_in)
        ng.links.new(g_in("Image"), a_in)
        ng.links.new(g_in("Fog Color"), b_in)
        oi = gout.inputs.get("Image")
        if oi is not None:
            ng.links.new(out_s, oi)
    mk = gout.inputs.get("Mask")
    if mk is not None:
        ng.links.new(m2.outputs[0], mk)
...
def _build_fog_group2(vfx, has_bg=True):
    ...
    def fogged(img_sock, f_sock, alpha_sock, y):
        """цвет слоя, затуманенный до альфа-овера; альфа сохраняется"""
...
def _build_fog_group3(vfx, has_bg=True):
    """Туман каждого слоя по СВОЕЙ mist-маске (корректная окклюзия),
    вся сборка внутри группы."""
    ng = bpy.data.node_groups.get("VFX_FogGroup")
    if ng is None:
        ng = bpy.data.node_groups.new("VFX_FogGroup", 'CompositorNodeTree')
    ng.nodes.clear()
    ...
    for layer in reversed(vfx.layers):
        ...
        ng.interface.new_socket(f"AL_{layer.id}", in_out='INPUT',
                                socket_type='NodeSocketFloat')
        ng.interface.new_socket(f"M_{layer.id}", in_out='INPUT',
                                socket_type='NodeSocketColor')
        ng.interface.new_socket(f"F_{layer.id}", in_out='INPUT',
                                socket_type='NodeSocketFloat')
        if layer.shadow_scene:
    ...
    def layer_fac(mist_sock, f_sock, y):
        mr = _new_node(ng, "ShaderNodeMapRange", "CompositorNodeMapRange")
        if mr is None:
            return None
        mr.location = (-1000, y)
        try:
            ...
            pass
        ng.links.new(mist_sock, mr.inputs.get("Value"))
        ng.links.new(g_in("Ramp Black"), mr.inputs.get("From Min"))
        ng.links.new(g_in("Ramp White"), mr.inputs.get("From Max"))
        m1 = math_node('MULTIPLY', (-750, y))
        if m1 is None:
            return None
        ng.links.new(mr.outputs.get("Result"), m1.inputs[0])
        ng.links.new(g_in("Strength"), m1.inputs[1])
        m2 = math_node('MULTIPLY', (-600, y))
        if m2 is None:
            return None
        ng.links.new(m1.outputs[0], m2.inputs[0])
        ng.links.new(f_sock, m2.inputs[1])
        return m2.outputs[0]

    def fogged(img_sock, fac_sock, alpha_sock, y):
        mix, fac_in, a_in, b_in, out_s = _fog_mix_node(ng, (-350, y))
        if mix is None or fac_in is None or out_s is None:
            return img_sock
        ng.links.new(fac_sock, fac_in)
        ng.links.new(img_sock, a_in)
        comb = _new_node(ng, "CompositorNodeCombineColor", "ShaderNodeCombineColor")
        if comb is not None and sep is not None:
            comb.location = (-500, y - 150)
            ng.links.new(sep.outputs[0], comb.inputs[0])
            ng.links.new(sep.outputs[1], comb.inputs[1])
            ng.links.new(sep.outputs[2], comb.inputs[2])
            if alpha_sock is not None:
    ...
    def alpha_over(bg_sock, fg_sock, fac_sock=None, y=0):
        ao = _new_node(ng, "CompositorNodeAlphaOver")
        if ao is None:
            return bg_sock
        ao.location = (400, y)
        ...
        if len(img) >= 2:
            ...
            ng.links.new(fg_sock, img[1])
        vs = [s for s in ao.inputs if s.type == 'VALUE']
        if vs and fac_sock is not None:
            ng.links.new(fac_sock, vs[0])
        outs = [s for s in ao.outputs if s.type == 'RGBA']
        return outs[0] if outs else bg_sock
    ...
    mcur = None
    if has_bg:
        fb = math_node('MULTIPLY', (-750, y))
        if fb is not None:
            ng.links.new(g_in("Strength"), fb.inputs[0])
            ng.links.new(g_in("F_BG"), fb.inputs[1])
            fac_bg = fb.outputs[0]
        else:
            fac_bg = g_in("Strength")
        cur = fogged(g_in("BG Image"), fac_bg, None, y)
        mcur = fac_bg
        y -= 300

    for entry in meta:
        lid = entry["id"]
        fac = layer_fac(g_in(f"M_{lid}"), g_in(f"F_{lid}"), y)
        if fac is None:
            y -= 300
            continue
        fog_obj = fogged(g_in(f"OBJ_{lid}"), fac, g_in(f"AL_{lid}"), y)
        if cur is None:
            cur = fog_obj
            mcur = fac
        else:
            cur = alpha_over(cur, fog_obj, y=y)
            sub = math_node('SUBTRACT', (-100, y - 100))
            mul = math_node('MULTIPLY', (50, y - 100))
            add = math_node('ADD', (200, y - 100))
            if sub and mul and add and mcur is not None:
                ng.links.new(fac, sub.inputs[0])
                ng.links.new(mcur, sub.inputs[1])
                ng.links.new(sub.outputs[0], mul.inputs[0])
                ng.links.new(g_in(f"AL_{lid}"), mul.inputs[1])
                ng.links.new(mcur, add.inputs[0])
                ng.links.new(mul.outputs[0], add.inputs[1])
                mcur = add.outputs[0]
        if entry["shd"]:
            cur = alpha_over(cur, g_in(f"SHD_{lid}"),
                             fac_sock=g_in(f"SS_{lid}"), y=y - 120)
        y -= 300
    ...
    if oi is not None:
        ng.links.new(cur, oi)
    mk = gout.inputs.get("Mask")
    if mk is not None and mcur is not None:
        ng.links.new(mcur, mk)
...
def build_comp_assembly(vfx, master, nt=None):
    ...
    # туман включен: вся сборка (фог слоев ДО альфа-оверов) внутри группы
    if getattr(vfx, "use_fog", False):
        try:
            mist = _get_mist_socket(nt)
            if mist is not None:
                ...
                gnode.node_tree = ng
                print("VFX fog: group assigned, wiring...")
                ...
                for entry in meta:
                    ...
                    # своя live mist-маска на слой - корректная окклюзия
                    if lay.scene is not None:
                        mn = nt.nodes.get(f"VFX_MIST_{lid}")
                        if mn is None:
                            mn = nt.nodes.new("CompositorNodeRLayers")
                            mn.name = f"VFX_MIST_{lid}"
                            mn.label = f"MIST {lay.layer_name}"
                            mn.location = (-900, -300)
                        mn.scene = lay.scene
                        try:
                            if lay.scene.view_layers:
                                mn.layer = lay.scene.view_layers[0].name
                        except Exception:
                            pass
                        s = gi(f"M_{lid}")
                        if s is not None and mn.outputs.get("Mist"):
                            relink(s, mn.outputs["Mist"])
                    if entry["shd"]:
                ...
                linked_m = 0
                for s in list(gnode.inputs):
                    if s.name.startswith("M_"):
                        lid2 = s.name[2:]
                        mn = nt.nodes.get(f"VFX_MIST_{lid2}")
                        if mn is not None and mn.outputs.get("Mist") is not None:
                            relink(s, mn.outputs["Mist"])
                            linked_m += 1
                print("VFX fog: M links =", linked_m)

                oi = gnode.outputs.get("Image")
                if oi is not None:
                    current = oi
                    fog_done = True
                    print("VFX fog: group wired OK")
    ...
    # туман выключен (или группа не собралась): старая цепочка миксов
    if not fog_done:
...
def _setup_fog_passes(vfx, master):
